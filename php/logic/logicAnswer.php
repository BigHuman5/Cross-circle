...
function checkBD($link)
{
    ...
    for($i=3;$i<=7;$i=$i+2)
    {
        ...
        $query = "select who_win,COUNT(who_win) from statics WHERE type=$i and id_session = 'fad' GROUP by who_win";
        $result = mysqli_query($link,$query) or die(mysqli_error($link));
        while ($row = mysqli_fetch_array($result))
        {
    ...
    $json=
    [
        "g3c" => $count_who_win[3][1],
        "g3h" => $count_who_win[3][2],
        "g3d" => $count_who_win[3][3],
        "g5c" => $count_who_win[5][1],
        "g5h" => $count_who_win[5][2],
        "g5d" => $count_who_win[5][3],
        "g7c" => $count_who_win[7][1],
        "g7h" => $count_who_win[7][2],
        "g7d" => $count_who_win[7][3],
        "y3c" => $count_who_win_you[3][1],
        "y3h" => $count_who_win_you[3][2],
        "y3d" => $count_who_win_you[3][3],
        "y5c" => $count_who_win_you[5][1],
        "y5h" => $count_who_win_you[5][2],
        "y5d" => $count_who_win_you[5][3],
        "y7c" => $count_who_win_you[7][1],
        "y7h" => $count_who_win_you[7][2],
